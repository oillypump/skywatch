"""
Pure Python script: scraping AQI + weather dari iqair.com.

Tidak ada dependency ke Airflow maupun Snowflake sama sekali. Output-nya
murni pandas DataFrame, yang oleh script ini juga disimpan sebagai CSV
supaya gampang dicek hasilnya.

Dependency: requests, beautifulsoup4, pandas, pyyaml, pendulum
...
Atau dipakai sebagai modul:
    from aqi_scraper import scrape
    df = scrape()
"""
...
def _load_config() -> dict:
    ...
    Isinya: weather_map & daftar lokasi (province + cities).
    """
...
def scrape() -> pd.DataFrame:
    """
    Scrape semua kota yang ada di config.yaml.
    Return: pandas DataFrame (kolom UPPERCASE), satu baris per kota.
    """
    cfg = _load_config()
    ...
    logger.info("Total %d baris berhasil di-scrape.", len(df))
    return df
...
if __name__ == "__main__":
    ...
    #     f"aqi_weather_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    # )
